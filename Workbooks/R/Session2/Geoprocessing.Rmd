...
```{r, message=FALSE, warning=FALSE, error=FALSE}
elevation <- get_elev_raster(as_Spatial(SouthSantiam$basin), z = 9)
elevation
gages <- st_transform(gages, projection(elevation))
gages$elevation <- raster::extract(elevation, as_Spatial(gages))
...
SouthSantiamBasin <- st_transform(SouthSantiam$basin, projection(elevation))
